...
class FasterImageViewManager(reactContext: ReactApplicationContext) : SimpleViewManager<AppCompatImageView>() {
  ...
  fun setImageSource(view: AppCompatImageView, options: ReadableMap) {
    ...
    var requestBuilder = ImageRequest.Builder(reactContext)
    ...
    val imageLoader = reactContext.imageLoader;
    val request = requestBuilder
      .target(
        onStart = { placeholder ->
